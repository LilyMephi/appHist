...
DraggableAxisChartView::DraggableAxisChartView(QChart *chart,
...
void DraggableAxisChartView::mousePressEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    // Проверяем, находится ли курсор рядом с линией
    QPointF scenePos = chart()->mapToValue(event->pos());
    if (qAbs(scenePos.x() - axisLine->at(0).x()) <
        0.5) {  // Порог для захвата линии
      isDragging = true;
    }
  }
...
void DraggableAxisChartView::mouseMoveEvent(QMouseEvent *event) {
  if (isDragging) {
...
void DraggableAxisChartView::mouseReleaseEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    isDragging = false;
  }
